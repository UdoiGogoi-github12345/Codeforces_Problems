...
void solve() {
    ll n, a, b, c;
    cin >> n >> a >> b >> c;
    //dp[i] will store the maximum number of pieces of ith length
    //following the conditions
    dp[0] = 0; //o length wale ribbon se max pieces 0 hi bannege naa
    dp[a] = 1, dp[b] = 1, dp[c] = 1;

    for (ll i = 1; i < n + 1; i++) {
        if (i - a >= 0 and dp[i - a] != 0) { //i am checking if dp[i-a]!=0 because this remaining length
            //i.e i-a must also have  avalid answer if its zero that means not even a single piece of,a,b,c xists
            //which is against the given conditpn

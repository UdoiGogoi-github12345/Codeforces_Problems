...
bool cmp(const pair<ll, ll>& a, const pair<ll, ll>& b)
{
 ...
 //global variables declared so that it reduces arguments in function calls

 
 ll ok(ll a[],ll i,ll k,ll z){
 	if(i<0 or i>n or k==0 ){ //no mmoves left or out of bounds
 		return 0;
 	}
 	if(dp[i][z]!=-1){
 		return dp[i][z];

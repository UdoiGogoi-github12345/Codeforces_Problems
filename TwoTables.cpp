...
void solve() {
	...
	ll ans1 = INT_MAX, ans2 = INT_MAX, ans3 = INT_MAX, ans4 = INT_MAX;
	if (w + w1 <= W) {
		ans1 = min(ans1, max(0ll, w - x1));
		ans2 = min(ans1, max(0ll, x2 - (W - w)));
		ans3 = min(ans3, max(0ll, w - x1));
		ans4 = min(ans4, max(0ll, x2 - (W - w)));
	}
	if (h + h1 <= H) {
		ans1 = min(ans1, max(0ll, y2 - (H - h)));
		ans2 = min(ans1, max(0ll, y2 - (H - h)));
		ans3 = min(ans3, max(0ll, h - y1));
		ans4 = min(ans4, max(0ll, h - y1));
	}

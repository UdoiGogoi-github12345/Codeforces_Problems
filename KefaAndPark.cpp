...
void dfs(ll node, ll ct, ll maxi) {
	visited[node] = true;
	if (v[node] == 1) {
		ct++; //if theres an adjacent cat there its ginna increate the continuing ct
	}
	else {
		ct = 0; //else the contnuing count is gonna become 0
	}
	maxi = max(maxi, ct);
	bool isleaf = true;
	for (auto x : adj[node]) {

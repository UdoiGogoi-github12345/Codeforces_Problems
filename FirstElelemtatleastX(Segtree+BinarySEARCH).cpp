...
ll query(vll& a, ll ss, ll se, ll idx, ll val) {

	if (seg[idx] < val ) {//agar ye value hi less than val hain toh usme deep jaake kuch
		//phayda nahi
		return -1;
	}
	if (se == ss) {
	...
	ll mid = (ss + se) / 2;
	ll res = query(a, ss, mid, 2 * idx, val);//agar left se -1 return nua iska matlab ye nahi
	//ki result doesnt exist
	//lets see the right part
	if (res == -1) {
		res = query(a, mid + 1, se, 2 * idx + 1, val);
	}
	return res;

...
ll ok(string s, ll x, ll y) {
	...
	for (ll i = 0; i < s.length(); i++) {
		...
		else if (s[i] - '0' == y and f == 2) {
			res++;
			f = 1;
		}
	}
	if (x != y and res % 2 == 1) {
		//sippose string tu hoise 001230006
		//atya jodi toi aetu check nologau ki x!=y tente ki hbo:res count ahibo 5 ta and res-- hoi
		//hoi jabo 4ta but asolke saboloi gole guteigita 0 toi rakhibo paro
		//00000 aetu good string hoi already
		//so aetu check lagibo lagibo ki x!=y
		res--;
	}
	return res;

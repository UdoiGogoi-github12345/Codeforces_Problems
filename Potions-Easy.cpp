...
void solve(){
   ...
   //dp[i][j] is the maximum health achievable when you consider the first i potions and u have taken j potions out of those i potions
   ...
   for(ll i=0;i<=n;i++){
    for(ll j=0;j<=i;j++){
        if(i==0 and j==0){
            ...
            if(v[i-1]+dp[i-1][j-1]>=0){
              dp[i][j]=max(v[i-1]+dp[i-1][j-1],dp[i-1][j]);
            }
            else{
                dp[i][j]=dp[i-1][j];
            }

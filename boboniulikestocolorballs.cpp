...
void solve(){
   ...
   if(odd<=1){
   	...
   	odd=0;
   	if(r&&g&&b){ //neither of them are zero
   		r--;g--;b--;w=w+3;
        if(r%2!=0)
        	odd++;
        if(g%2!=0)
        ...
        if(w%2!=0)
        	odd++;
        if(odd<=1){ //atmost 1 odd only allowed to be placed in the middle of palindrome
   		    cout<<"Yes"<<nl;
   		    return;
   	    }
    }
   cout<<"No"<<nl; //else in all cases output is no only
   return;

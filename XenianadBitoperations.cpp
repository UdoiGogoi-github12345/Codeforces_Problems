...
//this is a sgement tree problem
...
void buildtree(ll s, ll e, ll index) {
	...
	if (seg[2 * index].idx) {
		seg[index].val = seg[2 * index].val | seg[2 * index + 1].val;
		seg[index].idx = 0;
	}
	else {
		seg[index].val = seg[2 * index].val ^ seg[2 * index + 1].val;
...
void update(ll a, ll b, ll s, ll e, ll index) {
	if (a<s or a>e) {
		return;
	}
	if (s == e) {
	...
	ll mid = (s + e) / 2;
	update(a, b, s, mid, 2 * index);
	update(a, b, mid + 1, e, 2 * index + 1);
	if (seg[2 * index].idx) {
		seg[index].val = seg[2 * index].val | seg[2 * index + 1].val;
		seg[index].idx = 0;
	}
...
void solve() {
	...
	for (ll i = 1; i <= m; i++) {
		...
		//point update
		update(a, b, 1, (1 << n), 1);
		cout << seg[1].val << nl; //segment tree ka pehla value
	}
}
int main()
{

...
void solve(){
   ...
   fo(i,0,n){
   	cin>>a[i];
   } //prefix sum wont work because she chooses ana rbitrary book and tries to read the books one by one starting from that id
   ll i=0,j=0,sum_t=0,ct=0,res=INT_MIN;
   while(i<n){
   	if(sum_t+a[i]<=t){
       sum_t+=a[i];
       ct++;i++;res=max(ct,res);

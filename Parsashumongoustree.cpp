...
ll dp[100005][2];
//dp[x][0] will store the maximum value of the subtree of x if u cghoose its beauty value as l[x]i.e left wala
//dp[x][1] will store the maximum value of the subtree of x if u cghoose its beauty value as r[x]i.e right wala
void dfs(ll node, ll par) {
	...
	for (auto x : adj[node]) {
		if (x != par) {
			dfs(x, node);
			dp[node][0] += max(abs(l[node] - l[x]) + dp[x][0], abs(l[node] - r[x]) + dp[x][1]);
			dp[node][1] += max(abs(r[node] - l[x]) + dp[x][0], abs(r[node] - r[x]) + dp[x][1]);
		}
	}
}
void solve() {
	...
	ll u, v;
	fill(adj + 1, adj + n + 1, vector<ll>());//important
	for (ll i = 1; i <= n - 1; i++) {
		cin >> u >> v;
		adj[u].pb(v);
		adj[v].pb(u);

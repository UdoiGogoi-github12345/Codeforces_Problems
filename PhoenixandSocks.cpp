...
void solve() {
	...
	ll ans = 0, ans1 = 0;
	vll umatchedl, umatchedr;
	for (auto it = mpr.begin(); it != mpr.end(); it++) {
		if (mpl.find(it->ff) != mpl.end()) {
			ll mn = min(mpl[it->ff], mpr[it->ff]);
			mpl[it->ff] -= mn;
			mpr[it->ff] -= mn;
		}
		ans += (mpr[it->ff] / 2);//remaining pairs if any
		for (ll i = 0; i < mpr[it->ff]; i++) {
			umatchedr.pb(it->ff);
		}
	}
	for (auto it = mpl.begin(); it != mpl.end(); it++) {
		ans1 += mpl[it->ff] / 2;
		for (ll i = 0; i < mpl[it->ff]; i++) {
			umatchedl.pb(it->ff);
		}
	}
	ll diff = abs((ll)(umatchedr.size()) - (ll)(umatchedl.size()));
	if (diff == 0) {
		cout << umatchedr.size() << nl;
		return;
	}
	if (umatchedr.size() > umatchedl.size()) {
		if (2 * ans >= diff) {
			cout << umatchedl.size() + diff / 2 << nl;
		}
		else {
			cout << ans + (diff - ans * 2) + umatchedl.size() << nl;
		}
		return;
	}
	if (umatchedl.size() > umatchedr.size()) {
		if (2 * ans1 >= diff) {
			cout << umatchedr.size() + diff / 2 << nl;
		}
		else {
			cout << ans1 + (diff - ans1 * 2) + umatchedr.size() << nl;
		}
		return;
	}
}

...
void solve() {
  ...
  for (ll i = 0; i < stools.size(); i++) {
    if (i <= k - 2) {
      ans += 0.5 * stools[i].ff; //first k-1 stools ko le lia hamne //aur k-1 expensive wale
      mp[kart].pb(stools[i].ss);
      kart++;
    }
    else {
      ans += stools[i].ff;
  ...
  if (stools.size() >= k) {
    for (ll i = k - 1; i < stools.size(); i++) {
      mn = min(mn, stools[i].ff);
    }
    for (ll i = 0; i < pencils.size(); i++) {
      mn = min(mn, pencils[i].ff);
    }
    ans -= 0.5 * mn;
  }
  cout << sp(ans, 1) << nl;
  ...
  for (int i = 1; i <= min((ll)(mp.size()), k - 1); i++) {
    //we only put stools in these k-1 carts
    cout << (ll)(mp[i].size()) << " ";
    for (auto it : mp[i]) {
      cout << it << " ";
    }
  ...
  //remaining kart is either 1 or k-stools.size();
  if (stools.size() < k) {
    deb(kart);
    ll idx = 0;
    for (ll i = kart; i <= k - 1 and idx < pencils.size(); i++) {
      mp[kart].pb(pencils[idx++].ss); kart++;
    }
    for (ll ok = idx; ok < pencils.size(); ok++)
      mp[kart].pb(pencils[ok].ss);

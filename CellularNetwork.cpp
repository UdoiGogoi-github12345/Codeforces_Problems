...
void solve(){  //two pointer
 ll int n,m,ans=INT_MIN;
    cin>>n>>m;
    ll a[n],b[m];
    ...
    for(ll int i=0;i<n;i++)
    {
        while(j+1<m && abs(a[i]-b[j])>=abs(a[i]-b[j+1])){ //for each city calculating the fartheste reach //simialr to sliding widow
        j++;
        }
        chmax(ans,abs(a[i]-b[j]));
    }
    cout<<ans;

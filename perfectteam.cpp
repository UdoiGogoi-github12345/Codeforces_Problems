...
void solve(){
  ...
  ll sum=c+m+x;
  ll teams=0;
  if(k==c){ //if cis the no of teams,and each team contains three members so 3*c is total no of members -c i.e 2c remaining mebers must be greater than or equal to 2*c
  	if((sum-k)>=(2*c)){ 
  		teams=c;
  	}
  	else{
